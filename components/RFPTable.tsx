            ...
            <table className="min-w-full divide-y divide-gray-200 hidden md:table">
                ...
                <thead className="bg-gray-50">
                    <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Budget / Deadline</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {rfpList.map((rfp) => (
                        <tr key={rfp.id} className="hover:bg-gray-50 transition duration-150">
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{rfp.title}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ${rfp.budget ? rfp.budget.toLocaleString() : 'TBD'} / {rfp.deadline ? new Date(rfp.deadline).toLocaleDateString() : 'N/A'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusMap[rfp.status].color}`}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                ...
                                {/* 💡 CHANGE: Draft Status triggers Modal */}
                                {rfp.status === 'draft' && (
                                    <button 
